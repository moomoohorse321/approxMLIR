...
#pragma omp parallel
{
	...
	while ((! cell_converged) && (iterations < max_iterations)) {
      ...
      //@APPROX LABEL("IMGVF_comp_out") APPROX_TECH(MEMO_IN|MEMO_OUT) IN(total_diff) OUT(total_diff)
      {
      //@APPROX LABEL("IMGVF_comp_in") APPROX_TECH(MEMO_IN|MEMO_OUT) IN(new_val) OUT(new_val)
        {
