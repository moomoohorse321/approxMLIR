...
using GlobalOrdinalType = int;
using LocalOrdinalType = int;

using MatrixType = CSRMatrix<ScalarType,LocalOrdinalType,GlobalOrdinalType>;
using VectorType = Vector<ScalarType,LocalOrdinalType,GlobalOrdinalType>;
struct matvec_std {
            ...
            VectorType& y)
{
	...
	MINIFE_GLOBAL_ORDINAL row_start = 0;
	MINIFE_GLOBAL_ORDINAL row_end   = 0;
        int NUM_TEAMS = atoi(std::getenv("NUM_BLOCKS"));
        int TPB = atoi(std::getenv("THREADS_PER_BLOCK"));

        #pragma omp target teams distribute parallel for num_teams(NUM_TEAMS) num_threads(TPB) firstprivate(row_start, row_end)
        for(MINIFE_LOCAL_ORDINAL row = 0; row < rows_size+255; ++row) {
                row_start = Arowoffsets[row];
                row_end   = Arowoffsets[row+1];

                MINIFE_SCALAR sum = 0;
                //@APPROX LABEL("whole_row_out") APPROX_TECH(MEMO_OUT) IN(row_start) OUT(sum)
                //@APPROX LABEL("whole_row_in") APPROX_TECH(MEMO_IN) IN(Acoefs[0:row_end-row_start]) OUT(sum)
                {
                  if(row < rows_size)
                  {
		  #pragma unroll(27)
