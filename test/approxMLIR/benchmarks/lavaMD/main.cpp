...
int main(  int argc, char *argv [])
{
  ...
  int *a = new int[1000];
  #pragma omp target data map(tofrom:a[:1000])
  {
  ...
  int num_teams = dim_cpu_number_boxes;
  int teamsize = std::atoi(std::getenv("THREADS_PER_BLOCK"));

  double start = omp_get_wtime();
  double compute_start, compute_end;
  printf("Num teams is: %d\n", num_teams);
...
#pragma omp barrier
          ...
          while(wtx < NUMBER_PAR_PER_BOX){
            ...
            //@APPROX LABEL("entire_box_out") APPROX_TECH(MEMO_OUT) IN(j) OUT(v,x,y,z)
            //@APPROX LABEL("entire_box_in") APPROX_TECH(MEMO_IN) IN(rv_cpu[first_j:4*NUMBER_PAR_PER_BOX], qv_cpu[first_j:4*NUMBER_PAR_PER_BOX]) OUT(v,x,y,z)
            {
              for (j=0; j<NUMBER_PAR_PER_BOX; j++){
                //@APPROX LABEL("particle_box_out") APPROX_TECH(MEMO_OUT) IN(j) OUT(vij, fxij, fyij, fzij)
                {
                  r2 = rv_cpu[first_i+wtx].v + rv_cpu[first_j+j].v - DOT(rv_cpu[first_i+wtx],rv_cpu[first_j+j]);
                  u2 = a2*r2;
                  vij= exp(-u2);
                  fs = 2*vij;
                  d.x = rv_cpu[first_i+wtx].x  - rv_cpu[first_j+j].x;
                  fxij=fs*d.x;
                  d.y = rv_cpu[first_i+wtx].y  - rv_cpu[first_j+j].y;
                  fyij=fs*d.y;
                  d.z = rv_cpu[first_i+wtx].z  - rv_cpu[first_j+j].z;
                  fzij=fs*d.z;



		  v +=  qv_cpu[first_j+j]*vij;
		  x +=  qv_cpu[first_j+j]*fxij;
		  y +=  qv_cpu[first_j+j]*fyij;
		  z +=  qv_cpu[first_j+j]*fzij;
	    ...
	    fv_cpu[first_i+wtx].v +=  v;
	    fv_cpu[first_i+wtx].x +=  x;
	    fv_cpu[first_i+wtx].y +=  y;
	    fv_cpu[first_i+wtx].z +=  z;
...
#pragma omp barrier
  ...
  double compute_elapsed = compute_end-compute_start;
  printf("Device offloading time: %f\n", end-start); 
  printf("Only compute: %f\n", compute_elapsed);
  ...
  printf("Number ofitems: %ld, size of the val: %lu\n", dim_cpu.space_elem, sizeof(dim_cpu.space_elem));
  myFile.write((char*)&dim_cpu.space_elem, sizeof(dim_cpu.space_elem));
  for(i=0; i<dim_cpu.space_elem; i=i+1){
    myFile.write((char*)&fv_cpu[i].v, sizeof(fp));
    myFile.write((char*)&fv_cpu[i].x, sizeof(fp));
    myFile.write((char*)&fv_cpu[i].y, sizeof(fp));
    myFile.write((char*)&fv_cpu[i].z, sizeof(fp));
  }
  myFile.close();
#endif
