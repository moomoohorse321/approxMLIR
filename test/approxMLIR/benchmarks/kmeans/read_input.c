...
void usage(char *argv0) {
	const char *help =
		...
		"    -c converg_max   :maximum number of iterations for convergence [default=500]\n"
...
int setup(int argc, char **argv) {
	...
	int converg_max = 500;			/* default value */
	...
	while ( (opt=getopt(argc,argv,"i:t:m:n:l:c:bro"))!= EOF) {
		switch (opt) {
			...
			case 'l': nloops = atoi(optarg);
				  break;
			case 'c': converg_max = atoi(optarg);
				  break;
			case '?': usage(argv[0]);
				  break;
    ...
    int tmp;
    size_t tmp1;
    read(infile, &tmp1, sizeof(size_t));
    read(infile, &tmp, sizeof(int));
	...
	index = cluster(npoints,				/* number of data points */
			...
			converg_max);           /* maximum number of iterations before failure to converge*/
