#!/usr/bin/env python3
"""Probe whether a real Stage 2(a) bad case can be pushed onto an INT8 fast path."""
...
def main() -> int:
    ...
    from kernels.quant_kernels import (
        ...
        int8_matmul_kernel,
        pack_b_i8_per_group_fp16_storage,
    ...
    c_bad = torch.empty_like(c_exact)
    bad_handle = approx_matmul_kernel_1[grid](
        a, b_packed, c_bad,
        M, N, K,
        a.stride(0), a.stride(1),
        group_size, 8,
        c_bad.stride(0), c_bad.stride(1),
        BLOCK_M=block_m, BLOCK_N=block_n, BLOCK_K=block_k,
    )
    ...
    bad_max_diff = float((c_bad - c_exact).abs().max().item())
    bad_mean_diff = float((c_bad - c_exact).abs().mean().item())
    ...
    bad_ms = _bench_ms(torch, bad_call, iters=200)
